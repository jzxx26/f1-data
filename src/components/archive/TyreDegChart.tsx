...
export function TyreDegChart({ laps, stints, drivers, isLoading }: TyreDegChartProps) {
  const scatterData = useMemo(() => {
    ...
    const validTimes = laps
      .filter((l) => driverNums.has(l.driver_number) && l.lap_duration && l.lap_duration > 0)
    ...
    laps.forEach((lap) => {
      ...
      const compound = resolveCompound(driverStints, lap.lap_number) ?? "UNKNOWN";
  ...
  const compounds = useMemo(() => [...new Set(scatterData.map((p) => p.compound))], [scatterData]);
    ...
    return (
      <div className="min-w-[200px] rounded-xl border border-white/10 bg-black/90 p-3 text-xs text-white/80 shadow-xl backdrop-blur-sm">
        <div className="mb-2 flex items-center justify-between">
          ...
          <span className="text-[10px] uppercase tracking-wider text-white/50">Lap {p.lap}</span>
        </div>
        <div className="grid grid-cols-2 gap-1.5 text-[11px]">
          <span className="text-white/50">Lap Time</span>
          <span className="text-right font-mono text-white/80">{formatTime(p.lapTime)}</span>
          <span className="text-white/50">Compound</span>
          <span className="flex items-center justify-end gap-1.5">
            <span className="h-2 w-2 rounded-full" style={{ backgroundColor: getTyreColor(p.compound) }} />
            {p.compound}
          </span>
  ...
  return (
    <div className="relative w-full overflow-hidden rounded-3xl border border-white/5 bg-zinc-900/60 shadow-[0_0_0_1px_rgba(255,255,255,0.03)]">
      <div className="flex flex-col gap-1 border-b border-white/5 bg-zinc-900/40 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.3em] text-white/50">Race Pace</p>
          <h2 className="text-lg font-semibold text-white">Tyre Degradation</h2>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          {compounds.map((c) => (
            <span key={c} className="flex items-center gap-1.5 rounded-full bg-black/40 border border-white/10 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-white/60">
              <span className="h-2 w-2 rounded-full" style={{ backgroundColor: getTyreColor(c) }} />
              {c}
            </span>
          ))}
      ...
      <div className="relative w-full px-2 bg-zinc-950/50" style={{ height: "360px" }}>
        {!hasData ? (
          <div className="flex items-center justify-center text-xs text-white/40" style={{ height: "360px" }}>
            {isLoading ? "Loading degradation data..." : "No race pace data available"}
          </div>
        ) : (
          <ChartContainer height={360}>
              <ScatterChart margin={{ top: 15, right: 20, left: 10, bottom: 10 }}>
                <CartesianGrid stroke="rgba(255,255,255,0.06)" strokeDasharray="3 3" />
                <XAxis dataKey="lap" type="number" name="Lap" stroke="#9ca3af" tickLine={false} axisLine={{ stroke: "rgba(255,255,255,0.15)" }} fontSize={11} tick={{ fill: "#9ca3af" }} label={{ value: "Lap Number", position: "insideBottomRight", offset: -5, fill: "#6b7280", fontSize: 10 }} />
                <YAxis dataKey="lapTime" type="number" name="Lap Time" stroke="#9ca3af" tickLine={false} axisLine={{ stroke: "rgba(255,255,255,0.15)" }} fontSize={11} tick={{ fill: "#9ca3af" }} tickFormatter={(v: number) => formatTime(v)} domain={["dataMin - 1", "dataMax + 1"]} />
                <ZAxis range={[30, 30]} />
                <Tooltip cursor={{ strokeDasharray: "3 3", stroke: "rgba(255,255,255,0.2)" }} content={renderTooltip} wrapperStyle={{ outline: "none" }} />
                {Array.from(seriesGroups.entries()).map(([key, points]) => {
                  const first = points[0];
                  return (
                    <Scatter key={key} data={points} fill={getTyreColor(first.compound)} fillOpacity={0.8} strokeWidth={1} stroke={first.color} name={`${first.driverAcronym} · ${first.compound}`} />
                  );
                })}
                <Legend wrapperStyle={{ paddingTop: 8, color: "rgba(255,255,255,0.55)", fontSize: 10 }} iconSize={8} />
              </ScatterChart>
          </ChartContainer>
        )}
      </div>
